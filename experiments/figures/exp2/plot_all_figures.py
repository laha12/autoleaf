...
OUTPUT_DIR = r'c:\Users\86185\Desktop\autoleaf\experiments\figures\exp2'
...
# 实验路径配置
EXP2_RESNET_DIR = r'c:\Users\86185\Desktop\autoleaf\experiments\exp2_resnet50_complex\20260427_160710\20260427_160710'
EXP2_CONVNEXT_DIR = r'c:\Users\86185\Desktop\autoleaf\experiments\exp2_convnext_complex\20260427_160907\20260427_160907'

# 解析ResNet50数据
resnet_log = parse_training_log(os.path.join(EXP2_RESNET_DIR, 'training.log'))
resnet_metrics = parse_thesis_metrics(os.path.join(EXP2_RESNET_DIR, 'thesis_metrics.json'))

# 解析ConvNeXt数据
convnext_log = parse_training_log(os.path.join(EXP2_CONVNEXT_DIR, 'training.log'))
convnext_metrics = parse_thesis_metrics(os.path.join(EXP2_CONVNEXT_DIR, 'thesis_metrics.json'))

# 统一配色
COLOR_RESNET = '#4E79A7'   # 蓝色：ResNet50
COLOR_CONVNEXT = '#E15759'  # 红色：ConvNeXt
COLOR_GRID = '#E0E0E0'
COLOR_TEXT = '#333333'

print(f"ResNet50: {len(resnet_log['epochs'])} epochs, Top-1={resnet_metrics['Top-1 Acc (%)']}%, 收敛={resnet_metrics['收敛轮数']}轮")
print(f"ConvNeXt: {len(convnext_log['epochs'])} epochs, Top-1={convnext_metrics['Top-1 Acc (%)']}%, 收敛={convnext_metrics['收敛轮数']}轮")
...
resnet_stage1_epochs = infer_stage1_end(os.path.join(EXP2_RESNET_DIR, 'training.log'))
convnext_stage1_epochs = infer_stage1_end(os.path.join(EXP2_CONVNEXT_DIR, 'training.log'))

print(f"ResNet Stage1实际轮次: {resnet_stage1_epochs}")
print(f"ConvNeXt Stage1实际轮次: {convnext_stage1_epochs}")
...
max_epochs = max(len(resnet_log['epochs']), len(convnext_log['epochs']))

ax1 = axes[0]
ax1.plot(resnet_log['epochs'], resnet_log['val_acc'], color=COLOR_RESNET, linewidth=1.5,
         marker='s', markersize=3, markevery=2, label='ResNet50')
ax1.plot(convnext_log['epochs'], convnext_log['val_acc'], color=COLOR_CONVNEXT, linewidth=1.5,
         marker='o', markersize=3, markevery=2, label='ConvNeXt-Tiny')
ax1.axvline(x=resnet_stage1_epochs + 0.5, color=COLOR_RESNET, linestyle='--', linewidth=0.8, alpha=0.5)
ax1.axvline(x=convnext_stage1_epochs + 0.5, color=COLOR_CONVNEXT, linestyle='--', linewidth=0.8, alpha=0.5)
ax1.text(resnet_stage1_epochs + 0.7, 3, 'Stage 2', fontsize=8, color=COLOR_RESNET, rotation=90, va='bottom')
ax1.text(convnext_stage1_epochs + 0.7, 3, 'Stage 2', fontsize=8, color=COLOR_CONVNEXT, rotation=90, va='bottom')
...
ax2.plot(convnext_log['epochs'], convnext_log['val_loss'], color=COLOR_CONVNEXT, linewidth=1.5,
         marker='o', markersize=3, markevery=2, label='ConvNeXt-Tiny')
ax2.axvline(x=resnet_stage1_epochs + 0.5, color=COLOR_RESNET, linestyle='--', linewidth=0.8, alpha=0.5)
ax2.axvline(x=convnext_stage1_epochs + 0.5, color=COLOR_CONVNEXT, linestyle='--', linewidth=0.8, alpha=0.5)
ax2.text(resnet_stage1_epochs + 0.7, 0.2, 'Stage 2', fontsize=8, color=COLOR_RESNET, rotation=90, va='bottom')
ax2.text(convnext_stage1_epochs + 0.7, 0.2, 'Stage 2', fontsize=8, color=COLOR_CONVNEXT, rotation=90, va='bottom')
...
ax2.set_ylim(0, max(max(resnet_log['val_loss']), max(convnext_log['val_loss'])) * 1.1)
...
ax.plot(resnet_log['epochs'], resnet_log['val_acc'], color=COLOR_RESNET, linewidth=1.5,
        marker='s', markersize=3, markevery=1, label='ResNet50')
ax.plot(convnext_log['epochs'], convnext_log['val_acc'], color=COLOR_CONVNEXT, linewidth=1.5,
        marker='o', markersize=3, markevery=1, label='ConvNeXt-Tiny')
ax.axvline(x=resnet_stage1_epochs + 0.5, color=COLOR_RESNET, linestyle='--', linewidth=0.8, alpha=0.5)
ax.axvline(x=convnext_stage1_epochs + 0.5, color=COLOR_CONVNEXT, linestyle='--', linewidth=0.8, alpha=0.5)
...
# 动态确定放大范围
zoom_start = max(resnet_stage1_epochs, convnext_stage1_epochs) + 4
zoom_start_idx_res = min(zoom_start - 1, len(resnet_log['epochs']) - 1)
zoom_start_idx_conv = min(zoom_start - 1, len(convnext_log['epochs']) - 1)

ax_inset.plot(resnet_log['epochs'][zoom_start_idx_res:], resnet_log['val_acc'][zoom_start_idx_res:],
              color=COLOR_RESNET, linewidth=2.0, marker='s', markersize=4, markevery=1)
ax_inset.plot(convnext_log['epochs'][zoom_start_idx_conv:], convnext_log['val_acc'][zoom_start_idx_conv:],
              color=COLOR_CONVNEXT, linewidth=2.0, marker='o', markersize=4, markevery=1)
...
ax_inset.text(max_epochs - 1, resnet_final + 1, f'{resnet_final:.2f}%', fontsize=9, color=COLOR_RESNET, ha='right')
ax_inset.text(max_epochs - 1, convnext_final + 1, f'{convnext_final:.2f}%', fontsize=9, color=COLOR_CONVNEXT, ha='right')
...
resnet_acc_vals = [resnet_metrics['Top-1 Acc (%)'], resnet_metrics['Top-5 Acc (%)'], resnet_metrics['Macro-F1'] * 100]
convnext_acc_vals = [convnext_metrics['Top-1 Acc (%)'], convnext_metrics['Top-5 Acc (%)'], convnext_metrics['Macro-F1'] * 100]
...
ax4 = axes[1]
categories_eff = ['训练时间\n(s)', '收敛轮数', '推理速度\n(ms)']
resnet_eff_vals = [resnet_metrics['训练时间 (s)'], resnet_metrics['收敛轮数'], resnet_metrics['推理速度 (ms)']]
convnext_eff_vals = [convnext_metrics['训练时间 (s)'], convnext_metrics['收敛轮数'], convnext_metrics['推理速度 (ms)']]
...
raw_matrix = np.array([
    [resnet_metrics['Top-1 Acc (%)'], resnet_metrics['Top-5 Acc (%)'],
     resnet_metrics['Macro-F1'], 1/resnet_metrics['收敛轮数'], 1/resnet_metrics['训练时间 (s)']],
    [convnext_metrics['Top-1 Acc (%)'], convnext_metrics['Top-5 Acc (%)'],
     convnext_metrics['Macro-F1'], 1/convnext_metrics['收敛轮数'], 1/convnext_metrics['训练时间 (s)']],
])
...
label_data = [
    ...
    (f"{resnet_metrics['Macro-F1']:.3f}", f"{convnext_metrics['Macro-F1']:.3f}"),
    (f"{resnet_metrics['收敛轮数']} ep", f"{convnext_metrics['收敛轮数']} ep"),
    (f"{resnet_metrics['训练时间 (s)']:.0f}s", f"{convnext_metrics['训练时间 (s)']:.0f}s"),
]
...
# ResNet50的 train-val gap
resnet_gap = [t - v for t, v in zip(resnet_log['train_acc'], resnet_log['val_acc'])]
convnext_gap = [t - v for t, v in zip(convnext_log['train_acc'], convnext_log['val_acc'])]

ax.plot(resnet_log['epochs'], resnet_gap, color=COLOR_RESNET, linewidth=1.5,
        marker='s', markersize=3, markevery=2, label='ResNet50')
ax.plot(convnext_log['epochs'], convnext_gap, color=COLOR_CONVNEXT, linewidth=1.5,
        marker='o', markersize=3, markevery=2, label='ConvNeXt-Tiny')
...
ax.plot(resnet_log['epochs'], resnet_log['val_top5'], color=COLOR_RESNET, linewidth=1.5,
        marker='s', markersize=3, markevery=2, label='ResNet50')
ax.plot(convnext_log['epochs'], convnext_log['val_top5'], color=COLOR_CONVNEXT, linewidth=1.5,
        marker='o', markersize=3, markevery=2, label='ConvNeXt-Tiny')

ax.axvline(x=resnet_stage1_epochs + 0.5, color=COLOR_RESNET, linestyle='--', linewidth=0.8, alpha=0.5)
ax.axvline(x=convnext_stage1_epochs + 0.5, color=COLOR_CONVNEXT, linestyle='--', linewidth=0.8, alpha=0.5)
ax.text(resnet_stage1_epochs + 0.7, 5, 'Stage 2', fontsize=8, color=COLOR_RESNET, rotation=90, va='bottom')
ax.text(convnext_stage1_epochs + 0.7, 5, 'Stage 2', fontsize=8, color=COLOR_CONVNEXT, rotation=90, va='bottom')
...
ax.plot(resnet_log['epochs'], resnet_log['train_loss'], color=COLOR_RESNET, linewidth=1.5,
        marker='s', markersize=3, markevery=2, label='ResNet50')
ax.plot(convnext_log['epochs'], convnext_log['train_loss'], color=COLOR_CONVNEXT, linewidth=1.5,
        marker='o', markersize=3, markevery=2, label='ConvNeXt-Tiny')

ax.axvline(x=resnet_stage1_epochs + 0.5, color=COLOR_RESNET, linestyle='--', linewidth=0.8, alpha=0.5)
ax.axvline(x=convnext_stage1_epochs + 0.5, color=COLOR_CONVNEXT, linestyle='--', linewidth=0.8, alpha=0.5)
ax.text(resnet_stage1_epochs + 0.7, 0.5, 'Stage 2', fontsize=8, color=COLOR_RESNET, rotation=90, va='bottom')
ax.text(convnext_stage1_epochs + 0.7, 0.5, 'Stage 2', fontsize=8, color=COLOR_CONVNEXT, rotation=90, va='bottom')

ax.set_xlabel('训练轮次 (Epoch)', fontsize=12)
ax.set_ylabel('训练损失 (Train Loss)', fontsize=12)
ax.set_title('训练损失收敛曲线对比', fontsize=13, fontweight='bold')
ax.set_xlim(1, max_epochs)
ax.set_ylim(0, max(max(resnet_log['train_loss']), max(convnext_log['train_loss'])) * 1.1)
ax.grid(True, linestyle='-', alpha=0.4, color=COLOR_GRID)
...
resnet_stab = resnet_metrics.get('stability', {})
convnext_stab = convnext_metrics.get('stability', {})

resnet_stab_vals = [
    resnet_stab.get('overall_variance', 0),
    resnet_stab.get('local_variance', 0),
    resnet_stab.get('max_fluctuation', 0),
    resnet_stab.get('late_stage_variance', 0),
]
convnext_stab_vals = [
    convnext_stab.get('overall_variance', 0),
    convnext_stab.get('local_variance', 0),
    convnext_stab.get('max_fluctuation', 0),
    convnext_stab.get('late_stage_variance', 0),
]

x = np.arange(len(stability_categories))
bars1 = ax.bar(x - width/2, resnet_stab_vals, width, label='ResNet50', color=COLOR_RESNET, edgecolor='white', linewidth=0.5)
bars2 = ax.bar(x + width/2, convnext_stab_vals, width, label='ConvNeXt-Tiny', color=COLOR_CONVNEXT, edgecolor='white', linewidth=0.5)
...
ax.set_yscale('log')
ax.set_ylim(0.1, max(max(resnet_stab_vals), max(convnext_stab_vals)) * 2)
ax.legend(frameon=True, fancybox=False, edgecolor='#CCCCCC')
